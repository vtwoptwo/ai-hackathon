...
def get_barrier(document_name:str) -> str:
   with open(document_name) as f:
    ...
    prompt = f'''In a term sheet, a "barrier" is a pre-established condition, often indicated as a percentage, that must be achieved for specific terms to become active. For instance, if there is a 65% barrier, it means that a particular provision or benefit takes effect only when a related metric or target reaches 65% of its designated objective.
                Regarding your task, you applied regex to create a dictionary where the keys are values followed by "%" in the term sheet, and each key has a list of sentences where it appears. To determine the barrier, you simply need to return the numerical value associated with the term "barrier" in this dictionary. Here you have an example:
                ##dictionary
                {example}
                ##answer
                Barrier: 65
                ...
                ##
                Barrier:'''
    barrier = llm_call(llm, prompt)
    return str(barrier.strip())
